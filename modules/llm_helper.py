...
def create_query_with_deepseek(
    ...
    -----------
    query: str
        The user's prompt to generate a Cypher query for.
    api_key: Optional[str]
        Optional Deepseek API key. If not provided, reads from DEEPSEEK_API_KEY env var.
...
def classify_with_deepseek(
    ...
    options: List[str]
        The list of option labels to choose from.
    api_key: Optional[str]
        Optional Deepseek API key. If not provided, reads from DEEPSEEK_API_KEY env var.
    timeout: int default=10
        Request timeout in seconds.
